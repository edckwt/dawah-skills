 ...
 *
 */
class html_ds_helper extends app_ds_helpers {
	...
	public function Input($type = 'text', $attr = array()) {
		...
		switch ($type) {
			...
			default :
				{
					if(empty($attr['value'])){
						$attr['value'] = set_value(OPICDS_Input_SLUG.$attr['name']);
					}
					 
					 
					$html .= sprintf('<input type="%s" name="%s" value="%s" id="%s" />', $type, OPICDS_Input_SLUG.$attr['name'], $attr['value'], $attr['id']);
	...
	protected function _select($attr = array()) {
		...
		$option = get_option(OPICDS_Input_SLUG.$get_attr_name);
		
		$html = sprintf('<select name="%s" id="%s" >', OPICDS_Input_SLUG.$get_attr_name, $get_attr_id);
	...
	protected function _radio($attr = array()) {
		foreach ($attr['options'] as $key => $value) {
			$checked = NULL;
			$html .= sprintf('<p><input type="radio" name="%s" id="%s" value="%s"  %s /> <span>%s</span></p>', OPICDS_Input_SLUG.$attr['name'], $attr['id'], $key, $checked, $value);
		}
	...
	protected function _checkbox($attr = array()) {
		...
		$option = get_option(OPICDS_Input_SLUG.str_replace('[]','',$get_attr_name));
		$html = '';
		foreach ($get_attr_options as $key => $value) {
			if(is_array($option) && in_array($key, $option)){
			...
			$html .= sprintf('<p><input type="checkbox" name="%s" id="%s" value="%s" %s /> <span>%s</span></p>', OPICDS_Input_SLUG.$get_attr_name, $get_attr_id, $key, $checked, $value);
		}
