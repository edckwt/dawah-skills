...
class app_ds_models {
	...
	{
		global $opicds_categories_lang;
		$return = array();
		$table = $this->wpdb->prefix.'options';
		$like = OPICDS_Input_SLUG."category_".get_option(OPICDS_Input_SLUG.'language').'_';
		...
		if(!empty($data) && is_array($data)){
			foreach ($data as $key => $value) {
				...
				$return[$key]['option_url'] = $opicds_categories_lang[get_option(OPICDS_Input_SLUG.'language')][$return[$key]['option_slug']]['importurl'];
